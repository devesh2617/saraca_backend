    ...
    try {
        if (file) {
            ...
            if (caseStudyFound.img) {
                fs.rmSync(path.join(__dirname, `../../public/images/caseStudies/${caseStudyFound.img.split("/")[caseStudyFound.img.split("/").length - 1]}`));
            }
...
const addEvent = asyncHandler(
    async (req: any, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        next(new Error('Error creating event: ' + error.message));
      }
    }
  );

  const editEvent = asyncHandler(
    async (req: any, res: Response, next: NextFunction) => {
        ...
        if (images) {
          ...
          for (const imagePath of existingEvent.images) {
            ...
            if (fs.existsSync(fullPath)) {
              fs.unlinkSync(fullPath); // Synchronously delete the image file
            }
          }
